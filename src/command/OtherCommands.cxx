...
static sonyPassInfo sonypassvalue;
static void
...
void handle_set_streamKey(std::string &p, std::string &vec)
{
	if(p == "noEkey")
	{
		sonypassvalue.pass.clear();
	}
	else
	{
		sonypassvalue.pass = p;
	}
	sonypassvalue.ivec = vec;
	FormatNotice(othercommand_domain, "set %s, %s", sonypassvalue.pass.substr(0,10).c_str(), sonypassvalue.ivec.c_str());
}

void handle_get_streamKey(std::string &p, std::string &vec)
{
	p = sonypassvalue.pass;
	vec = sonypassvalue.ivec;
}

void handle_set_qqTrackType(std::string &qqTrackType)
{
	sonypassvalue.qqTrackType = qqTrackType;
}

void handle_get_qqTrackType(std::string &qqTrackType)
{
	qqTrackType = sonypassvalue.qqTrackType;
}