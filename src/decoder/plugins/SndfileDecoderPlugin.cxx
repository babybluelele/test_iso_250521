		   ...
		   TagHandler &handler) noexcept
{
	...
	if (value != nullptr)
	{
		...
		strncpy(buf, p, 2048);
		buf[2048 - 1] = '\0';
		if(0 == convert_isUTF8(buf, strlen(buf)))
		{
			if(0 == convert_convertstr(buf, strlen(buf), utf, 2048))
			{
				handler.OnTag(tag, utf);
			}
			else
			{
		...
		if(str == 4 && strncmp(value, "?", 1) == 0) return false;
	}
	return true;
}
...
sndfile_scan_stream(InputStream &is, TagHandler &handler)
{
	...
	{
		if(!sndfile_handle_tag(sf, i.str, i.tag, handler))
		{
			handler.OnClear();
			return true;
		}
